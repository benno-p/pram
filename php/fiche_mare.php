...
,epci.l_nom as l_nom_epci
  FROM 
   $caracterisations c 
    LEFT JOIN $localisations l ON (c.loc_id_plus = l.loc_id_plus)
    LEFT JOIN (select id_plus, string_agg(photo_link || '__' ||to_char(date_photo,'DD-MM-YYYY'),'|') as photo_link from $photos group by 1) as  p ON (c.loc_id_plus = p.id_plus)
    LEFT JOIN $communes2018 com ON st_intersects(l.loc_geom, com.l_geom)
    LEFT JOIN $epci2018 epci ON st_intersects(l.loc_geom, epci.l_geom)
  where car_id = '".$_POST['id_car_']."' ";
...
$photos_lines = '';
if ($arr["photo_link"] != "") {
    $photos = explode("|", $arr["photo_link"]);
    foreach ($photos as $value){
        $phto = explode("__",$value)[0];
        $phtd = explode("__",$value)[1];
        $photos_lines .= "<tr><td> --------------------------------------- </td></tr><tr><td><p>Le ".$phtd." :</p><img src='http://cen-normandie.com/".$dir_."/img/photos/".$phto."' style='width:80%;text-align: left;'></td></tr>";
    }
}
                ...
                <table style='width:100%;'> 
                    <tr>                
                        <td style='width:20%;text-align:left;font-size:14px;'>
                            <img src='../img/pram_normandie.PNG' style='width:80%'>
                        </td>
...
if (file_exists ($root_dir_."/pdf/fiche_mare.pdf")) {
    //delete last file
    unlink($root_dir_."/pdf/fiche_mare.pdf");
};


//write file on serveur
$html2pdf->output($root_dir_."/pdf/fiche_mare.pdf","F");
echo "fiche_mare.pdf";
