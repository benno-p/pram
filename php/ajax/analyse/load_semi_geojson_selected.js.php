...
if ($_POST['table_name'] == ' layers.zones_specifiques ') {
    $sql = "
...
FROM ( SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features
FROM (SELECT 'Feature' As type
   , ST_AsGeoJSON( st_transform(ST_SnapToGrid(lg.l_geom, 0.00007),4326) )::json As geometry
   , row_to_json(lp) As properties
  FROM ".$_POST["table_name"]." As lg 
        INNER JOIN (SELECT l_id, l_nom, '".$_POST["table_name"]."' as table_name FROM ".$_POST["table_name"]." WHERE l_id = '".$_POST["id"]."' ) As lp 
