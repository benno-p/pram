...
$status_1 = pg_result_status($result);
pg_close($dbconn);
...
if ($status_1 < 3) {
    ...
    $result = pg_prepare($dbconn, "sql", "SELECT MAX(loc_id), loc_id_plus, loc_uuid FROM $localisations WHERE loc_uuid::text = replace( $1::text, 'uuid:','') group by 2,3 order by 1 desc limit 1");
    $result = pg_execute($dbconn, "sql",array( $uuid_core ));
    $row = pg_fetch_array($result))
    $id_locplus_generated = $row["loc_id_plus"];
    $status_2 = pg_result_status($result);
    pg_close($dbconn);
    ...
    if($status_2 < 3) {
        ...
        $status_3 = pg_result_status($result);
        pg_close($dbconn);
        ...
        if($status_3 < 3) {
            ...
            $result = pg_execute($dbconn, "sql",array( $uuid_core ));
            $status_4 = pg_result_status($result);
            echo json_encode([$status_1, $status_2, $status_3, $status_4]);
            pg_close($dbconn);
        } else {
            echo 'La mare n\est pas importée dans la table caracterisation';
        }
    } else {
        echo 'La mare n\est pas importée dans la table localisation';
    }
} else {
    echo 'Les données du formulaire odk n\'ont pas été trouvées';
}
