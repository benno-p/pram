...
,epci.l_nom as l_nom_epci
  FROM 
   $caracterisations c 
    ...
    LEFT JOIN (select id_plus, string_agg(photo_link,'|') as photo_link from $photos where car is true group by 1) as  p ON (c.loc_id_plus = p.id_plus)
...
if ($arr["photo_link"] != "") {
    ...
    foreach ($photos as $value){
        $photos_lines .= "<tr><td><img src='".$value."' style='width:80%'></td></tr>";
    }
}
...
if (file_exists ("$root_/pdf/analyse_pdf.pdf")) {
    //delete last file
    unlink("$root_/pdf/analyse_pdf.pdf");
};

//write file on serveur
$html2pdf->output("$root_/pdf/analyse_pdf.pdf","F");
